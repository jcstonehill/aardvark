...
values0 = np.array([val/max(values) for val in values])

for i in range(300):
    values = values0*(i+1)/300
    dataset = {
        "fission" : values
    }
    ...
    while(len(suffix) < 3):
        suffix = "0" + suffix
    mesh.write_data_to_vtk("aardvark/nets2025/static_fission/My_File_" + suffix + ".vtk", dataset)
